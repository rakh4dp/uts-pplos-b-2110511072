...
const authController = {
        ...
        User.findByEmail(email, async (err, results) => {
            ...
            if (!user.password) {
                return res.status(400).json({ 
                    message: 'Akun ini terdaftar via Google. Silakan login menggunakan tombol Google Login.' 
                });
            }
    ...
    googleCallback: (req, res) => {
        const user = req.user; 
        const payload = { id: user.id, name: user.display_name, email: user.email, profile_pic: user.profile_pic };
